...
export async function writeAuditRecord(record: GuardianAuditRecord): Promise<void> {
  await (db as any).guardianCheck.create({
    data: {
      ...
      injectionBlocked: record.injectionBlocked,
      injectionAttackType: record.injectionAttackType,
      injectionDescription: record.injectionDescription,
      injectionMatchedText: record.injectionMatchedText,
